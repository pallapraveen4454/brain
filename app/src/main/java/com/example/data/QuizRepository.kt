...
class QuizRepository(
    ...
    companion object {
        private const val PREFS_NAME = "category_completion_prefs"
        // In-memory fallback for JVM unit testing when Context is null
        internal val inMemoryCompletedCategories = ConcurrentHashMap<String, String>()
    }
    ...
    /**
     * Map raw category IDs to canonical keys.
     */
    ...
    /**
     * Detects if the category is Daily Challenge.
     */
    fun isDailyChallenge(categoryId: String): Boolean {
        val canonical = getCanonicalCategoryKey(categoryId)
    ...
    /**
     * Calculates the next calendar date (YYYY-MM-DD) for availability display.
     */
    fun getNextAvailableDateString(todayDateOverride: String? = null): String {
    ...
    private fun getAccountKey(): String {
        return try {
            ...
            if (store.isGuestActive()) {
                store.getGuestId()
            } else {
                val auth = com.google.firebase.auth.FirebaseAuth.getInstance()
                auth.currentUser?.uid ?: store.getGuestId()
            }
        } catch (e: Exception) {
            "default_user"
        }
    ...
    /**
     * Check if a category has already been completed today.
     * Persisted across app restarts, force close, and relaunch for both guest and auth users.
     */
    fun isCategoryCompletedToday(categoryId: String, dateOverride: String? = null): Boolean {
        val canonicalKey = getCanonicalCategoryKey(categoryId)
        val today = dateOverride ?: getTodayDateString()
        val prefs = getPrefs()
        val accountKey = getAccountKey()

        // 1. Check local persistent completion preferences
        val savedDate = prefs?.getString("completed_${accountKey}_${canonicalKey}", null)
            ?: prefs?.getString("completed_${canonicalKey}", null)
            ?: inMemoryCompletedCategories["${accountKey}_${canonicalKey}"]
            ?: inMemoryCompletedCategories[canonicalKey]

        if (savedDate == today) {
            return true
        }

        // 2. Cross-check in profile quiz history for genuine completion on today's calendar date
        try {
            val profileStore = UserProfileStore(context)
            val history = profileStore.getProfile().quizHistory
            val completedInHistory = history.any { result ->
                val resultKey = getCanonicalCategoryKey(result.categoryName)
                if (resultKey != canonicalKey) return@any false
                val resultDate = if (result.dateFormatted.length >= 10) {
                    result.dateFormatted.substring(0, 10)
                } else {
                    SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(Date(result.timestamp))
                }
                resultDate == today
            }
            if (completedInHistory) {
                recordCategoryCompletion(categoryId, today)
                return true
            }
        } catch (e: Exception) {
            Log.e("QuizRepository", "Error checking quizHistory for category completion", e)
        }
    ...
    /**
     * Record a category as completed for today.
     * Returns true if newly recorded, or false if it was already recorded today (prevents duplicates).
     */
    fun recordCategoryCompletion(categoryId: String, dateOverride: String? = null): Boolean {
        val canonicalKey = getCanonicalCategoryKey(categoryId)
        val today = dateOverride ?: getTodayDateString()
        val prefs = getPrefs()
        val accountKey = getAccountKey()

        val alreadySaved = (prefs?.getString("completed_${accountKey}_${canonicalKey}", null) == today) ||
                (prefs?.getString("completed_${canonicalKey}", null) == today) ||
                (inMemoryCompletedCategories["${accountKey}_${canonicalKey}"] == today) ||
                (inMemoryCompletedCategories[canonicalKey] == today)

        if (alreadySaved) {
            Log.d("QuizRepository", "Category '$canonicalKey' was already recorded as completed on $today.")
            return false
        }

        prefs?.edit()
            ?.putString("completed_${canonicalKey}", today)
            ?.putString("completed_${accountKey}_${canonicalKey}", today)
            ?.apply()

        inMemoryCompletedCategories[canonicalKey] = today
        inMemoryCompletedCategories["${accountKey}_${canonicalKey}"] = today
        Log.d("QuizRepository", "Recorded category completion for '$canonicalKey' on $today.")
        return true
    }

    /**
     * Returns set of all canonical category keys completed today.
     */
    fun getCompletedCategoriesForToday(dateOverride: String? = null): Set<String> {
        val today = dateOverride ?: getTodayDateString()
        val categories = listOf("gk", "science", "history", "sports", "movies", "tech", "geo", "math", "daily")
        return categories.filter { isCategoryCompletedToday(it, today) }.toSet()
    }

    /**
     * Reset all daily category completion records.
     */
    fun resetDailyCategoryCompletions() {
        try {
            getPrefs()?.edit()?.clear()?.apply()
            inMemoryCompletedCategories.clear()
            Log.d("QuizRepository", "resetDailyCategoryCompletions: Cleared all daily category completion records.")
        } catch (e: Exception) {
            Log.e("QuizRepository", "resetDailyCategoryCompletions failed", e)
        }
    }
}
